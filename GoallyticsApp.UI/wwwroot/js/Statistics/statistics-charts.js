...
function initializeStatisticsCharts(chartData, marketData, insightsData) {
    ...
    // Confidence accuracy chart kaldırıldı
...
function updateMarketCard(marketId, data) {
    ...
    if (correctEl) correctEl.textContent = data.correct.toString();
...
function updateInsights(insights) {
    ...
    if (bestMarketEl) bestMarketEl.textContent = insights.bestMarket;
    if (bestMarketAccEl) bestMarketAccEl.textContent = '%' + insights.bestMarketAccuracy.toFixed(1);
    if (bestLeagueEl) bestLeagueEl.textContent = insights.bestLeague;
    if (bestLeagueAccEl) bestLeagueAccEl.textContent = '%' + insights.bestLeagueAccuracy.toFixed(1);
